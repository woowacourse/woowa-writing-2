...
우테코에 와서 처음으로 JAVA API 코드를 열어보았다. 예전에는 아예 볼 생각을 못 했다. 어딘가 무서웠다. 하지만 크루들이 API 코드를 자연스럽게 들여다보는 모습을 보며 바뀌었다. JAVA나 Spring 코드를 들여다보면, 작동 원리를 확실하게 알 수 있다. 그리고 "좋은" 코드를 접할 수 있다는 큰 장점이 있다. 
